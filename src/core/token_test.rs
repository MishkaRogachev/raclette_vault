...
mod tests {
    ...
    use crate::core::{chain::Chain, token::{Token, TokenList}};
    ...
    fn test_flattern_token_list() {
        ...
        let mainnet_tokens = tokens.iter().map(|token| token.clone().with_chain_data(
            Chain::EthereumMainnet, dummy_address, mainnet_decimals)).collect::<TokenList>();

        let sepolia_decimals = 14;
        let sepolia_tokens = tokens.iter().map(|token| token.clone().with_chain_data(
            Chain::EthereumSepolia, dummy_address, sepolia_decimals)).collect::<TokenList>();

        let arbitrum_decimals = 15;
        let arbitrum_tokens = tokens.iter().map(|token| token.clone().with_chain_data(
            Chain::ArbitrumMainnet, dummy_address, arbitrum_decimals)).collect::<TokenList>();
        ...
        for token in final_list {
            ...
            assert_eq!(token.chain_data.get(&Chain::EthereumMainnet).unwrap().decimals, mainnet_decimals);
            assert_eq!(token.chain_data.get(&Chain::EthereumSepolia).unwrap().decimals, sepolia_decimals);
            assert_eq!(token.chain_data.get(&Chain::ArbitrumMainnet).unwrap().decimals, arbitrum_decimals);
        }
    }
}
